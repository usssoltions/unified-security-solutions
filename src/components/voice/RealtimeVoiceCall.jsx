...
export default function RealtimeVoiceCall({ 
  ...
  const startRingtone = () => {
    ...
    try {
      // Use a loud, alarming ringtone from a CDN
      ringtoneAudio.current = new Audio('https://actions.google.com/sounds/v1/alarms/alarm_clock.ogg');
      ...
      if ('vibrate' in navigator) {
        ...
        vibratePattern();
        ringtoneInterval.current = setInterval(vibratePattern, 3000);
      }
        ...
        if (!peerConnections.current[participantId]) {
          ...
          pc.onconnectionstatechange = () => {
            ...
            if (pc.connectionState === 'connected') {
              ...
              console.warn('Disconnected from', participantId);
              setConnectedParticipants(prev => prev.filter(id => id !== participantId));
            }
          };
          ...
          pc.oniceconnectionstatechange = () => {
            ...
            if (pc.iceConnectionState === 'connected' || pc.iceConnectionState === 'completed') {
              ...
              console.error('ICE connection failed for', participantId);
            }
          };
