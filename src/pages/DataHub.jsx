...
export default function DataHub() {
  // Effective tenant app name for the emailed report footer (white-label).
  const { user } = useAuth();
  ...
  const sendEmail = async () => {
    ...
    setEmailStatus("");
    // HTML ESCAPING — every exported value is untrusted display data and is
    // escaped before interpolation into the email body.
    const esc = (v) => String(v ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
    const rows = filtered.slice(0, 100).map(r =>
      `<tr>${config.fields.map(f => `<td style="padding:6px 10px;border-bottom:1px solid #e2e8f0;">${esc(fmt(r[f]))}</td>`).join("")}</tr>`
    ).join("");
    const body = `
...
<p style="color:#94a3b8;font-size:11px;margin-top:20px;">Generated by ${brandName} — Data Hub</p>
</body></html>`;
    try {
